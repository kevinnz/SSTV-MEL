...
public final class SSTVDecoderCore {
    ...
    private let visDetector = VISDetector()
    ...
    /// Minimum samples needed to attempt VIS detection
    private static let minSamplesForVIS: Int = 88200  // ~2 seconds at 44.1kHz
    ...
    ///
    /// Clears all accumulated samples and state but preserves configuration
    /// (sample rate, mode, options, delegate).
    public func reset() {
        resetState()
        self.mode = nil
    ...
    private func processSignalSearch() {
        ...
        // Find signal start
        imageStartSample = findSignalStart(
        ...
        // Report sync lock
        let confidence: Float = 0.9  // High confidence if we found consistent sync
    ...
    /// Find the start of the SSTV signal in frequency data
    ///
    ...
    private func findSignalStart(
        ...
        for startIndex in stride(from: skipSamples, to: searchLimit, by: searchStep) {
            ...
            // Accept if we have at least 6 valid consecutive frames
            if validFrames >= 6 {
                return fineTuneSyncStart(
